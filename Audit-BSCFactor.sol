 ...
 *   2) Choose one of the tariff plans, enter the BNB amount (0.01 BNB minimum) using our website "Harvest" button
...
contract BSCFactor {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 5e16; // 0.01 bnb
	...
	function withdraw() public {
		User storage user = users[msg.sender];
        // 		require (user.checkpoint.add(TIME_STEP)<block.timestamp,"Only 1 withdrawal is allowed in 24hrs");
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (user.checkpoint < finish) {
				...
				if (from.add(TIME_STEP) < block.timestamp && block.timestamp < finish) {
